...
with open('data_vel.csv','w', newline = '') as f:
    writer = csv.writer(f)
    writer.writerow(['time','goal_velocity','present_velocity'])
    while True:
        ...
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, dxl_id, 600, target_velocity_u32)
        ...
        while time.time() - t_start < 3.0:
            present_velocity, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, dxl_id, 615)
            ...
            writer.writerow([t,target_velocity,present_velocity])
